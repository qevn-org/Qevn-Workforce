...
async def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    logger.info(f"Dynamic Planner analyzing goal: '{goal}' (Loop {loop_count}/{max_loops})")
    ...
    if loop_count > max_loops:
        logger.error(f"Planner loop limit exceeded ({loop_count}/{max_loops}). Suspending workflow.")
        EventBus.publish("IncidentTriggered", {
        ...
        return {
            ...
            "context_data": {
                ...
                "action_inputs": {"reason": f"Loop iteration count {loop_count} exceeds threshold."}
            }
        }
        
    if accumulated_cost >= cost_budget:
        logger.error(f"Planner cost budget exceeded (${accumulated_cost}/${cost_budget}). Suspending workflow.")
        EventBus.publish("IncidentTriggered", {
            "workflow_instance_id": run_id_str,
            "severity": "Critical",
        ...
        return {
            ...
            "context_data": {
                ...
                "action_inputs": {"reason": f"Accumulated cost ${accumulated_cost} exceeds budget ${cost_budget}."}
            }
        }
    ...
    if openai_key or anthropic_key:
        try:
            ...
            class PlannerDecision(BaseModel):
                reasoning: str = Field(description="Explanation of the planning decision and why this step was chosen.")
                next_action: str = Field(description="Name of the action to execute (e.g., 'search_web', 'sync_lead', 'send_email'), or 'finalize' if the goal is fully accomplished.")
                action_inputs: Dict[str, Any] = Field(default_factory=dict, description="The input parameters matching the action's schema. Leave empty if next_action is 'finalize'.")
                current_plan: List[str] = Field(default_factory=list, description="An updated list of steps to achieve the user's goal.")
            
            # Load employee configurations for prompts
            emp_config = EmployeeLoader.load_from_db(state["employee_id"], state["organization_id"])
            ...
            if openai_key:
                ...
                model = ChatOpenAI(model=os.getenv("OPENAI_MODEL", "gpt-4o"), temperature=0)
            else:
                from langchain_anthropic import ChatAnthropic
                model = ChatAnthropic(model_name=os.getenv("ANTHROPIC_MODEL", "claude-3-5-sonnet-20240620"), temperature=0)
            ...
            if decision.next_action == "finalize":
                ...
                messages.append({"role": "assistant", "content": f"Supervisor decided to finalize. Reasoning: {decision.reasoning}"})
                return {
                    "messages": messages,
                    "next_node": "finalize",
                ...
                messages.append({"role": "system", "content": f"Planner decided to run action: {decision.next_action}. Reasoning: {decision.reasoning}"})
                return {
                    "loop_count": loop_count,
            ...
            logger.error(f"Error in Supervisor LLM reasoning (falling back to mock): {str(e)}")

    # Fallback / Mock Planning transitions
    if not completed:
        next_action = "search_web"
        action_inputs = {"query": goal}
        messages.append({"role": "system", "content": f"Planner (Mock) decided to run action: {next_action}"})
        return {
            "loop_count": loop_count,
...
def capability_router(state: AgentState) -> str:
    ...
    EventBus.publish("PolicyDecisionLogged", {
    ...
    if decision == PolicyDecision.REJECT:
        raise PermissionError(f"Policy Engine rejected execution of '{action}': {decision_res['reason']}")
...
async def human_approval_gate(state: AgentState) -> Dict[str, Any]:
    ...
    await WorkflowRuntimeEngine.pause_for_approval(run_id, checkpoint_id, action, inputs)
...
async def capability_executor_node(state: AgentState) -> Dict[str, Any]:
    ...
    EventBus.publish("CapabilityStarted", {
    ...
    EventBus.publish("CapabilityCompleted", {
        "workflow_instance_id": str(run_id),
        "capability_id": capability.definition.id,
        "success": res["success"],
        "error": res["error"]
    ...
    messages.append({"role": "system", "content": f"Capability {capability.definition.id} output: {res['output']}"})
