...
def get_current_user(
    ...
    try:
        ...
        if token.startswith("mock_") or token == "mock_jwt":
            payload = {"sub": "usr_mock_123", "org_id": "org_mock_456", "role": "admin"}
        else:
            if not jwk_client:
                raise HTTPException(
                    status_code=503, detail="JWKS client is not initialized."
                )
        ...
        if not user_id or not org_id:
            raise HTTPException(
                status_code=401,
                detail="Invalid session token claims. Sub and org_id are required.",
            )
