...
def install_employee_capabilities(id: str, payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
...
def start_workflow(payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
...
def resume_workflow(id: str, payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
...
async def webhook_ingress(subscription_id: str, request: Request):
    ...
    if not signature:
        raise HTTPException(status_code=401, detail="Webhook validation failed: Missing signature header.")
        
    expected_sig = hmac.new(secret_key.encode("utf-8"), body, hashlib.sha256).hexdigest()
    if not hmac.compare_digest(expected_sig, signature):
        # In production this writes to webhook_dlq table
        logger = logging.getLogger("WebhookIngress")
        logger.warning(f"Signature mismatch. Writing payload to DLQ for subscription {subscription_id}")
        raise HTTPException(status_code=401, detail="Webhook verification signature mismatch.")
...
async def get_scorecard(employee_id: str, current_user: AuthContext = Depends(get_current_user)):
    try:
        scorecard = await AIEvaluator.generate_scorecard(employee_id, current_user.org_id)
        return {"success": True, "data": scorecard}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/evaluation/experiments", response_model=Dict[str, Any])
async def create_experiment(payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
        # In production this parses payload and inserts to experiments table
        return {"success": True, "message": "Experiment successfully created and activated."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/evaluation/replay", response_model=Dict[str, Any])
async def replay_workflow(payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
...
async def submit_human_feedback(payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
        wf_id_str = payload.get("workflow_instance_id")
        if not wf_id_str:
            raise ValueError("Missing workflow_instance_id payload parameter.")
...
async def run_benchmark_regression(suite_id: str, payload: Dict[str, Any], current_user: AuthContext = Depends(get_current_user)):
    try:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({"status": "subscribed", "conversation_id": conversation_id})
