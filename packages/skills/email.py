...
class SendEmailSkill(BaseSkill):
    ...
    def _execute(self, validated_inputs: SendEmailInput, credentials: dict = None) -> dict:
        # Load Tool integration
        gmail = ToolRegistry.get_tool("GmailTool", credentials=credentials or {"token": "mock"})
        res = gmail.run({
