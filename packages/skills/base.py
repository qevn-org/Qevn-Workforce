...
class BaseSkill(ABC):
    ...
    @abstractmethod
    def _execute(
        self, validated_inputs: BaseModel, credentials: Dict[str, Any] = None
    ) -> Any:
        pass

    def execute(
        self, inputs: Dict[str, Any], credentials: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        validated = self.validate_inputs(inputs)
        result = self._execute(validated, credentials)

        output_model = (
            self.output_schema(**result) if isinstance(result, dict) else result
        )
        return (
