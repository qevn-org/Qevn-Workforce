...
class PolicyEngine:
    ...
    def evaluate(cls, context: PolicyContext) -> Dict[str, Any]:
        logger.info(f"Evaluating action '{context.action}' for Employee {context.employee_id}...")
        ...
        if required_perm and context.user_role == "restricted":
            logger.warning(f"Action '{context.action}' rejected due to insufficient role permissions.")
            return {
