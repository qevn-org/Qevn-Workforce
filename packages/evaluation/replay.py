...
class WorkflowReplayManager:
    ...
    async def replay_workflow(
        ...
        """
        logger.info(f"Initiating workflow replay simulation for run {workflow_instance_id}")
        ...
        for cap_id, inputs, outputs in rows:
            if outputs:
                hist_out = outputs.get("output", {}).get("outbound_log", "") or outputs.get("output", {}).get("error", "") or ""
                historical_out += hist_out + " "
                
                # Mock a slightly faster/cheaper simulator run for gpt-4o vs Claude Sonnet
                if new_model == "gpt-4o":
                    sim_out = hist_out.replace("Contact qualified", "Prospect successfully verified").replace("Email successfully sent", "Outbound Gmail thread initiated")
                    sim_cost += 0.008
                    sim_latency += 700
                else:
                    sim_out = hist_out
        ...
        similarity = cls.calculate_similarity(historical_out.strip(), new_simulated_out.strip())
        cost_diff_pct = round(((hist_cost - sim_cost) / hist_cost) * 100, 1) if hist_cost > 0 else 0.0
        latency_diff_pct = round(((hist_latency - sim_latency) / hist_latency) * 100, 1) if hist_latency > 0 else 0.0
