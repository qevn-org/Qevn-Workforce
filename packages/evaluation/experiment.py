...
class ExperimentManager:
    ...
    async def route_execution(
        cls, employee_id: str, organization_id: str, rand_val: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        if roll < split:
            ...
            logger.info(
                f"Routing execution to Challenger variant for experiment '{exp['name']}' (split={split})"
            )
            return {
                "in_experiment": True,
            ...
            logger.info(
                f"Routing execution to Champion variant for experiment '{exp['name']}' (split={split})"
            )
            return {
                "in_experiment": True,
                "experiment_id": exp["id"],
    ...
    async def simulate_shadow_run(
        cls, employee_id: str, organization_id: str, input_goal: str
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Executing Shadow Mode run for '{exp['name']}' in org '{organization_id}'"
        )
