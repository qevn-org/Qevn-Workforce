...
class WorkflowRuntimeEngine:
    ...
    @classmethod
    async def save_checkpoint(cls, run_id: uuid.UUID, capability_id: str, inputs: Dict[str, Any], outputs: Optional[Dict[str, Any]], state_snapshot: Dict[str, Any]):
        ...
        EventBus.publish("CheckpointCreated", {
    ...
    @classmethod
    async def execute_with_retry(cls, run_id: uuid.UUID, operation_callable: callable, max_retries: int = 3, backoff_base: float = 2.0) -> Any:
        ...
        while attempt < max_retries:
            try:
                ...
                EventBus.publish("RetryTriggered", {
                    "workflow_instance_id": str(run_id),
                    "attempt": attempt,
                    "error": str(e)
                })
                ...
                if attempt == max_retries:
                    ...
                    EventBus.publish("WorkflowFailed", {"workflow_instance_id": str(run_id), "error": str(e)})
                    raise e
    ...
    @classmethod
    async def pause_for_approval(cls, run_id: uuid.UUID, checkpoint_id: uuid.UUID, action_type: str, payload: Dict[str, Any]):
        instance = cls._active_runs.get(run_id)
        if instance:
            instance.status = WorkflowStatus.PAUSED
            
        EventBus.publish("ApprovalRequested", {
            "workflow_instance_id": str(run_id),
            "checkpoint_id": str(checkpoint_id),
            "action_type": action_type,
            "payload": payload
        })
        logger.info(f"Workflow {run_id} execution paused awaiting human approval for {action_type}.")

    @classmethod
    async def resume_workflow(cls, run_id: uuid.UUID, approval_decision: str) -> Optional[WorkflowInstance]:
        instance = cls._active_runs.get(run_id)
        if not instance:
            return None
        ...
        EventBus.publish("ApprovalGranted" if approval_decision == "approved" else "ApprovalRejected", {
            "workflow_instance_id": str(run_id)
        })
