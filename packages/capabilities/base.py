...
class BaseCapability(ABC):
    ...
    def validate_inputs(self, inputs: Dict[str, Any]) -> BaseModel:
        ...
        try:
            ...
            raise ValueError(f"Input validation failed for capability {self.definition.name}: {str(e)}")

    @abstractmethod
    def _execute(self, validated_inputs: BaseModel, context: Dict[str, Any] = None) -> Any:
    ...
    def execute(self, inputs: Dict[str, Any], context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Capability {self.definition.name} execution started with inputs: {inputs}")
        try:
            validated = self.validate_inputs(inputs)
            result = self._execute(validated, context)
            
            # Post-execution validation
            output_model = self.definition.output_schema(**result) if isinstance(result, dict) else result
            
            return {
                "success": True,
                "output": output_model.model_dump() if isinstance(output_model, BaseModel) else output_model,
                "error": None
            }
        except Exception as e:
            logger.error(f"Capability {self.definition.name} execution failed: {str(e)}")
            return {
                "success": False,
                "output": None,
                "error": str(e)
