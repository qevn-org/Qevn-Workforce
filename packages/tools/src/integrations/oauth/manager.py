    ...
    async def get_valid_token(cls, connection_id: str) -> str:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if expiry and now >= (expiry - timedelta(minutes=2)):
                logger.info(f"OAuth access token expired for connection {connection_id}. Initiating rotation...")
                
                # Fetch refresh token
                refresh_token = cls.decrypt_token(enc_refresh) if enc_refresh else None
                if not refresh_token:
                    raise PermissionError("Access token expired and no refresh token available.")
                    
                # Call mock token exchange API or actual OAuth Provider endpoint
                new_access, new_refresh, expires_in = await cls._refresh_provider_token(refresh_token)
                
                # Encrypt and update credentials in database
                new_enc_access = cls.encrypt_token(new_access)
                new_enc_refresh = cls.encrypt_token(new_refresh) if new_refresh else enc_refresh
                ...
                logger.info(f"OAuth connection credentials rotated successfully for {connection_id}.")
                return new_access
                
            return cls.decrypt_token(enc_access)

    @classmethod
    async def _refresh_provider_token(cls, refresh_token: str) -> tuple[str, Optional[str], int]:
        """
