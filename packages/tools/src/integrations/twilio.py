...
class TwilioCallSchema(BaseModel):
    ...
    voice_prompt: str = Field(description="Instructions or script for the text-to-speech engine")

class TwilioTool(BaseTool):
    name = "TwilioTool"
    description = "Triggers voice phone calls or SMS dispatches for escalation workflows"
    ...
    def _execute(self, validated_args: TwilioCallSchema) -> str:
        ...
        account_sid = self.credentials.get("account_sid") or os.getenv("TWILIO_ACCOUNT_SID")
        auth_token = self.credentials.get("auth_token") or os.getenv("TWILIO_AUTH_TOKEN")
        from_number = self.credentials.get("from_number") or os.getenv("TWILIO_FROM_NUMBER", "+1234567890")
