...
class BaseTool(ABC):
    ...
    def _check_rate_limit(self):
        ...
        if len(self._last_calls) >= self.rate_limit_per_minute:
            raise ToolRateLimitException(f"Rate limit exceeded for tool {self.name}. Max {self.rate_limit_per_minute}/min.")
        self._last_calls.append(now)

    def _validate_auth(self):
        """Checks if auth parameters exist in credentials."""
        if not self.credentials:
            raise ToolAuthException(f"No authentication parameters provided for {self.name}.")
