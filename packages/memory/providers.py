...
class ConversationMemoryProvider(IMemoryProvider):
    ...
    def store(self, entry: MemoryEntry, ttl: Optional[int] = 3600) -> bool:
        ...
        try:
            ...
            key = (
                f"org:{entry.organization_id}:employee:{entry.employee_id}:conversation"
            )
            ...
            logger.warning(
                f"Redis offline, falling back to local MemoryOS storage: {str(e)}"
            )
...
class TaskMemoryProvider(IMemoryProvider):
    ...
    def semantic_search(
        ...
        return [
            ...
            and query.lower() in e.content.lower()
        ][:limit]
